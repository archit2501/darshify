...
export function PlayButton({ playing = false, onClick, size = 48, label = "Play" }: {
  playing?: boolean; onClick?: (e: React.MouseEvent) => void; size?: number; label?: string;
}) {
  return (
    <button
      onClick={onClick}
    ...
    >
      {playing ? <PauseIcon size={size * 0.42} /> : <PlayIcon size={size * 0.46} />}
    </button>
  );
}
