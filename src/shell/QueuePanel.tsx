  ...
  return (
    <aside className="w-[300px] shrink-0 bg-panel rounded-lg m-2 p-4 overflow-y-auto hidden lg:block">
      ...
      {p.current && (
        <>
          ...
          <div className="flex items-center gap-3 mb-5 p-2 rounded-md bg-card-hi">
            <Art
              src={coverFor(p.current.kind)}
              gradient={p.current.gradient}
      ...
      <div className="flex flex-col">
        {upcoming.length === 0 && (
          <div className="text-sub text-sm">Nothing queued.</div>
        )}
        {upcoming.map((t, i) => (
          <button
            key={`${t!.id}-${i}`}
            onClick={() => p.jumpTo(p.pos + 1 + i)}
          ...
          >
            ...
            <div className="min-w-0">
              ...
              <div className="text-sub text-xs truncate">{t!.subtitle}</div>
            </div>
          </button>
        ))}
      </div>
