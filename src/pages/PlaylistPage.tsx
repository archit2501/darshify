...
export function PlaylistPage() {
  ...
  const tracks = pl.trackIds.map(trackById).filter(Boolean) as NonNullable<ReturnType<typeof trackById>>[];
    ...
    <div>
      <header className="flex flex-col md:flex-row md:items-end gap-4 md:gap-6 pt-4 pb-6 -mx-4 md:-mx-6 px-4 md:px-6"
        ...
        <div>
          ...
          <div className="text-sm mt-2"><span className="font-bold text-white">Darshil Jain</span> · {tracks.length} songs · {formatTime(total)}</div>
        </div>
      </header>

      <div className="flex items-center gap-6 py-4">
        <PlayButton size={56} playing={playing}
          onClick={() => (playing ? p.toggle() : p.play(tracks[0], pl.trackIds))} />
      </div>
      ...
      <div className="grid grid-cols-[24px_1fr_auto_auto] md:grid-cols-[24px_1.6fr_1fr_auto_auto] gap-4 px-4 py-2 text-sub text-xs border-b border-white/10 mb-2">
        <span>#</span><span>Title</span><span className="hidden md:block">Plays</span><span /><span className="text-right">⏱</span>
      </div>
      {tracks.map((t, i) => (
